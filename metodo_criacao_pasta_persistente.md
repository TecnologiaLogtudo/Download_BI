Para garantir que os arquivos sejam criados, salvos e gerenciados sem causar erros de permissão (PermissionError: [Errno 13]), o sistema utiliza uma estratégia de "Fail-safe" (Degradação Graciosa) combinada com um mapeamento em banco de dados para a persistência das informações.

Abaixo, detalho como isso funciona no seu código e a arquitetura envolvida:

1. Como a pasta exports evita erros de permissão (PermissionError)
A mágica acontece no arquivo webapp/server.py através da função _ensure_writable_dir. Durante a inicialização do servidor (startup), o sistema não apenas cria as pastas, mas também testa se tem permissão de escrita nelas antes de confirmar o uso.

O fluxo é o seguinte:

O sistema tenta criar a pasta principal (ROOT / "exports").
Ele tenta criar um arquivo temporário oculto chamado .write_test e escrever "ok" nele.
Se conseguir, ele deleta o arquivo teste e segue usando essa pasta principal.
Se der erro de permissão (PermissionError), a exceção é capturada no except Exception as exc: e o sistema automaticamente muda a rota para uma pasta de fallback (segurança) no diretório temporário do sistema operacional (/tmp/logtudo/exports), que geralmente tem permissões de escrita abertas.

"def _ensure_writable_dir(primary: Path, fallback: Path, label: str) -> Path:
    try:
        primary.mkdir(parents=True, exist_ok=True)
        probe = primary / ".write_test"
        probe.write_text("ok", encoding="utf-8") # Teste de escrita
        probe.unlink(missing_ok=True)
        return primary
    except Exception as exc:
        print(f"[startup] sem permissao para {label} em {primary}: {exc}")
        # FALLBACK: Se der erro, tenta no /tmp/ que costuma ter permissão livre
        fallback.mkdir(parents=True, exist_ok=True)
        probe = fallback / ".write_test"
        ...
        return fallback"

2. Como a persistência é garantida
A persistência dos dados é dividida em duas camadas (Física e Lógica):

Camada Física (Arquivos): Os arquivos gerados (planilhas processadas, capturas de tela do Playwright, logs, vídeos) são salvos diretamente no disco usando a biblioteca padrão do Python (pathlib). No caso de o sistema rodar dentro de um Docker, caminhos como /app/exports/ ou /tmp/ mantêm os arquivos vivos enquanto o container rodar. Para não perdê-los após o container ser desligado, a arquitetura pressupõe o uso de Volumes do Docker (montando a pasta do host na pasta do container).

Camada Lógica (Banco de Dados): No lugar de depender apenas do sistema de arquivos para saber "o que está lá", o sistema possui um banco de dados (provavelmente SQLite ou PostgreSQL) gerenciado pelo SQLAlchemy (webapp.models.JobArtifact). Sempre que um artefato é salvo na pasta, ele também é registrado no banco de dados.

Há uma inteligência na função _resolve_artifact_disk_path que consegue encontrar os arquivos exportados mesmo se a pasta raiz mudar ou se for um caminho de um container antigo:

"def _resolve_artifact_disk_path(file_path: str, job_id: Optional[str] = None) -> Optional[Path]:
    # 1) Caminho exato gravado no banco.
    # 2) Caminhos alternativos e relativos.
    # 3) Compatibilidade com caminhos antigos de container (ex: /app/exports/jobs/)
    ...
"

3. Arquitetura e Ferramentas Utilizadas
A arquitetura geral voltada para o motor de exportações é a seguinte:

FastAPI (Backend Web): Utilizado para prover endpoints rápidos que criam trabalhos assíncronos (Jobs) e fornecem links de download dos arquivos resultantes.
Pathlib (Python Standard Library): Utilizada no lugar do antigo os.path. É excelente para lidar com caminhos de forma "Cross-Platform" (funciona bem manipulando caminhos do Windows e do Linux/Docker sem quebrar barras \ e /).
SQLAlchemy (ORM de Banco de Dados): Gerencia as tabelas como JobRun, JobArtifact e JobError. Ele que permite manter a persistência e um histórico detalhado (relatório) independentemente dos arquivos na pasta de exportação.
Pandas & Openpyxl (SpreadsheetWriter): Quando a automação encerra (job.status == 'completed'), o Pandas converte a memória dos resultados e o Openpyxl atualiza as células do Excel em disco para construir os artefatos dentro do exports.
Playwright: A ferramenta de automação web tem configurações injetadas para armazenar seus artefatos nativos (vídeos .webm e traces .zip) diretamente dentro dos subdiretórios em exports/jobs/<id_do_job>.
Essa abordagem de Teste Ativo de Escrita (probe.write_text) combinada com Fallbacks (Caminho principal -> Caminho /tmp/) é uma excelente prática de Engenharia de Software para tornar aplicações tolerantes a falhas de infraestrutura (Resiliência).
